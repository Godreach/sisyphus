//! 占位模块：workspace（ADR-0009 已列；工作区布局/指令处理随 B3-T6 实现）。
//!
//! B3-T1 只提供分派骨架的接收端：WorkspaceCommand 指令经 channel reader
//! 投递到本模块通道，占位循环收帧即记日志（「占位 handle 可收」）。
...
/// workspace 占位句柄：持有下行接收端与收帧观测（分派骨架断言面）。
pub struct Handle {
...
impl Handle {
    ...
    /// 占位循环：收列表/清理指令即记日志并忽略（真实处理随 B3-T6）。
    pub async fn run(mut self) {
        while let Some(msg) = self.rx.recv().await {
            ...
            tracing::info!(?msg, "workspace 收到指令（占位：忽略，处理随后续批次）");
