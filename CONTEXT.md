...
Agent 的在线与否由通道连接状态判定：连接存活即在线，超时无心跳即离线。离线的 Agent 不接新任务；其运行中任务转入未知态（unknown）--Agent 侧继续执行，重连后回归运行态，超过宽限窗口（默认 10 分钟）未恢复才判失败。另有两种「在线但不可派发」状态：升级排空中与版本不兼容（窗口外，显示为不兼容而非离线）。
...
任务执行产生的文件或目录（二进制、报告、压缩包等），按任务级声明上传到本次构建的产物空间，供下载与本次构建内后续任务引用（不跨构建）。

### 产物空间（Artifact Space）

Server 为每次构建分配的隔离对象空间，容纳该构建各任务及各次执行 attempt 上传的产物。不同构建的产物空间互不覆盖；跨构建选择或获取产物不属于该概念。

### 产物集（Artifact Set）

任务的一条上传声明在某次执行 attempt 中产生的一组文件及其相对目录结构。文件或目录都形成产物集；只有整组文件上传并核验完成，产物集才可供浏览和下载。

### 制品库（Artifact Repository）

用于聚合浏览和管理各项目历史构建产物空间的可选顶级功能。它不是软件包仓库，不提供包管理协议或“最新制品”解析语义；未启用时不影响流水线的其它能力。
...
任务执行输出的带类型事件流，包含按顺序交织的 stdout/stderr 输出块与步骤生命周期事件，按任务 attempt 的序号定位。运行中保存在 Agent 本地，任务结束后形成日志归档；日志不是产物，不出现在制品库中。

### 日志归档（Log Archive）

一个任务 attempt 结束后封存的不可变日志事件记录，独立于运行期间 Agent 上的日志缓冲。归档是否可用与任务执行结果是两个不同状态。
...
Agent 的升级分发机制：管理员把 agent 发行包上传到 Server（仅上传、不自动拉 Releases，窗口外版本拒收），手动发全量或单台升级指令（持久化、离线补发）。Agent 收到即进入排空--停接新任务、等运行中任务全部终态（无限排空）--然后下载校验、原子换入二进制、spawn 新进程重启；旧二进制保留为 `.old`，连续 3 次启动失败自动退回。手动替换二进制永远兜底。窗口外（过旧）Agent 任务面拒连、升级面保留：不派任务但始终可收升级指令。

### 数据目录（Data Directory）

Server 的本地数据落点（`--data-dir`，默认 `./data`，Docker 固定 `/data`），内含 SQLite 数据库及未启用对象存储时的本地文件；首次启动生成带注释的 config.toml。启用外部对象存储后，产物与日志归档的正文可位于该后端，数据目录仍保存元数据与配置。配置优先级：CLI flag > `SISYPHUS_` 前缀环境变量 > config.toml > 内置默认。
