...
mod common;
use common::{
    TestApp, body_json, body_text, custom_req, req_with_cookie, setup_and_login, test_app,
};
...
async fn full_upgrade_roundtrip_upload_command_drain_download_version_update() {
    ...
    let Kind::Upgrade(cmd) = cmd_msg.kind.unwrap() else {
        panic!("应为 UpgradeCommand")
    };
...
async fn workspace_list_roundtrip_and_clean_delivered() {
    ...
    let resp = req_with_cookie(
        &app,
        "POST",
        "/api/v1/agents/linux-1/workspace/list",
        None,
        Some(&cookie),
    )
    .await;
    ...
    let resp = req_with_cookie(
        &app,
        "POST",
        "/api/v1/agents/linux-1/workspace/clean",
        Some(r#"{"pipeline": "demo"}"#.into()),
        Some(&cookie),
    )
    .await;
    assert_eq!(resp.status(), axum::http::StatusCode::ACCEPTED);
    ...
    let resp = req_with_cookie(
        &app,
        "POST",
        "/api/v1/agents/offline-1/workspace/list",
        None,
        Some(&cookie),
    )
    .await;
...
async fn cache_list_roundtrip_and_delete_delivered() {
    ...
    let resp = req_with_cookie(
        &app,
        "POST",
        "/api/v1/agents/linux-1/cache/list",
        None,
        Some(&cookie),
    )
    .await;
    ...
    let resp = req_with_cookie(
        &app,
        "POST",
        "/api/v1/agents/linux-1/cache/delete",
        Some(r#"{"key": "cargo-abc"}"#.into()),
        Some(&cookie),
    )
    .await;
    assert_eq!(resp.status(), axum::http::StatusCode::ACCEPTED);
