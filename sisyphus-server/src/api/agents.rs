//! Agent 注册面端点（票 B2c-T3，ADR-0008 最小注册面）：全局 admin 专属。
//!
//! - **建条目**（`POST /agents`）：签发 per-Agent token（`sisa_` 族，复用
//!   B2b token 基座：SHA-256 落库、明文只在创建响应出现一次）+ 一次性
//!   注册码（明文只在创建响应出现一次，库里只存哈希；注册码换 token 的
//!   完整流程随 Agent 批次，本批由建条目直接签发 token）。
...
//!   `custom_labels`。
//!
//! 权限：全局 admin（[`super::policy::RequireGlobalAdmin`]）——Agent 是全
//! 局资源，管理面不按项目分域（与用户管理同档）。系统标签不可手编：
//! 由注册/心跳上报（gRPC 面），本模块只读不回写。
//!
//! 审计（ADR-0015）：Agent 建立 + 注册码签发（`agent_created`）、停用即
//! 吊销 token（`agent_disabled`）、启用恢复（`agent_enabled`）——detail 记
//! 构建机名，token/注册码值永不落审计（与 PAT 同纪律）。
